...
#mycursor.execute("CREATE TABLE workout (id INT AUTO_INCREMENT PRIMARY KEY, userID INT,FOREIGN KEY (userID) REFERENCES user(id), workoutType VARCHAR(255), caloriesBurned INT, duration TIME, date DATE)")
#mycursor.execute("CREATE TABLE workout_set (id INT AUTO_INCREMENT PRIMARY KEY, workoutID INT, FOREIGN KEY (workoutID) REFERENCES workout(id), exerciseID INT, FOREIGN KEY (exerciseID) REFERENCES exercise(id), reps INT, weight INT)")
...
#mycursor.execute("CREATE TABLE goal (id INT AUTO_INCREMENT PRIMARY KEY, userID INT, FOREIGN KEY (userID) REFERENCES user(id), goal VARCHAR(255), progress DECIMAL(10, 10), deadline DATE)")

#mycursor.execute("CREATE TABLE interactionLog (id INT AUTO_INCREMENT PRIMARY KEY, userID INT, FOREIGN KEY (userID) REFERENCES user(id),message VARCHAR(255), response VARCHAR(255), dateAndTime DATETIME)")

#mycursor.execute("CREATE TABLE meal (id INT AUTO_INCREMENT PRIMARY KEY, userID INT,FOREIGN KEY (userID) REFERENCES user(id) ,mealType VARCHAR(255), calories INT, protien INT, carbs INT, fat INT, date DATE)")
